...
import com.youtubebing.stealthplayer.Bing.BingVideosLoader;
 ...
 */
public class BingSearchFragment extends BaseFragment implements ItemEventsListener<VideoItem> {
    ...
    public BingSearchFragment() {
        // Required empty public constructor
    }

    public static BingSearchFragment newInstance() {
        return new BingSearchFragment();
    }
    ...
    public void searchQuery(final String query) {
        ...
        getLoaderManager().restartLoader(2, null, new LoaderManager.LoaderCallbacks<List<VideoItem>>() {
            @Override
            public Loader<List<VideoItem>> onCreateLoader(final int id, final Bundle args) {
                return new BingVideosLoader(context, query);
            }
    ...
    public void onItemClick(VideoItem video) {
        ...
        itemSelected.onBingPlaylistSelected(searchResultsList, searchResultsList.indexOf(video));
